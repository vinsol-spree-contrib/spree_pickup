  ...
  <div class="panel-heading stock-location no-borderb" data-hook="stock-location">
    <h1 class="panel-title">
      ...
      <% if(@order.pickup? && (shipment.shipped_for_pickup? || shipment.ready?) && can?(:update, shipment)) %>
        <%= link_to "Pick up Ready", "javascript:;", class: "pickup_ready pull-right btn btn-success lm-5", data: { "shipment-number": shipment.number } %>
      <% end %>

      <% if(@order.pickup? && shipment.ready? && can?(:update, shipment)) %>
        <%= link_to "Shipped to Pickup Location", "javascript:;", class: "pickup_ship pull-right btn btn-success", data: { "shipment-number": shipment.number } %>
      <% end %>
      <br/><br/>
